...
define([
    ...
    function (
        ...
        LayerManagerHelper.loadServerCapabilites2 = function (serverAddress, wmsCapsDoc,layername) {
            var wmsService = wmsCapsDoc.service,
                wmsLayers = wmsCapsDoc.capability.layers,
                server = {
                    id: LayerManagerHelper.nextServerId++,
            ...
            // wmsLayers = wmsLayers[0].layers;
            // LayerManagerHelper.assembleLayers2(wmsLayers, server.layers,layername);

            return LayerManagerHelper.assembleLayers2(wmsLayers, server.layers,layername);;
        };
        /**
         *
        ...
        LayerManagerHelper.assembleLayers2 = function (wmsLayers, layerNodes,layername) {
            var layer;
            for (var i = 0; i < wmsLayers.length; i++) {
                if(wmsLayers[i].name==layername)
                {
                     layer = wmsLayers[i];
                      var isLayer = ko.observable(layer.name && layer.name.length > 0 || false),
                        node = {
                            title: layer.title,
                            abstract: layer.abstract,
                            layerCaps: layer,
                    ...
                    if (layer.layers && layer.layers.length > 0) {
                        this.assembleLayers2(layer.layers, node.layers);
                    }
                    // layerManager.addLayerFromCapabilities(layer, constants.LAYER_CATEGORY_OVERLAY);
                    // layerNode.check();
                    layerNodes.push(node);
                }
...
//                        this.timeSeriesPlayer.layer = null;
        ...
        LayerManagerHelper.applyOptionsToLayer2 = function (layer, options, category) {
            var opt = (options === undefined) ? {} : options;
