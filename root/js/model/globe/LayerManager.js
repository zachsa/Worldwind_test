    ...
    'url-search-params'],
        function (
            ...
            var LayerManager = function (globe) {
                 ...
                 */
                this.localWmsServer = "http://app01.saeon.ac.za:8082/geoserver/wms" //window.location.origin + "/geoserver/wms";
//                this.localWmsServer = window.origin + "/geoserver/gwc/service/wms";
                ...
                /** Data layers are shapes and markers. */
                this.polygonLayers = ko.observableArray();

                /** Data layers are shapes and markers. */
                this.imageLayers = ko.observableArray();
...
//                this.addBaseLayer(new WorldWind.BMNGLandsatLayer(), {enabled: false, detailControl: config.imagerydetailControl});
                this.addBaseLayer(new WorldWind.BingAerialLayer(null), {enabled: false, opacity: 0.7, detailControl: config.imagerydetailControl});
...
//            this.addAvailableWfsLayers();
            ...
            LayerManager.prototype.addDataLayer = function (layer, options) {
                var index = this.backgroundLayers().length + this.baseLayers().length + this.overlayLayers().length + this.effectsLayers().length
                    + this.polygonLayers().length +this.imageLayers().length +    this.dataLayers().length ,
                        layerProxy;
            ...
            LayerManager.prototype.addWidgetLayer = function (layer, options) {
                var index = this.backgroundLayers().length + this.baseLayers().length + this.overlayLayers().length + this.effectsLayers().length
                    + this.polygonLayers().length +this.imageLayers().length +this.dataLayers().length + this.widgetLayers().length,
                        layerProxy;
...
//            }
                if (wwLayer) {
                    ...
                    } else if (category === constants.LAYER_CATEGORY_POLYGONS) {
                        this.addDataLayer(wwLayer);
                    }else if (category === constants.LAYER_CATEGORY_IMAGE) {
                        this.addDataLayer(wwLayer);
                    }
            ...
            LayerManager.prototype.removeLayer = function (wwLayer) {
                ...
                if (category === constants.LAYER_CATEGORY_BASE) {
                    ...
                    this.polygonLayers.remove(pred);
                }else if (category === constants.LAYER_CATEGORY_IMAGE) {
                    this.imageLayers.remove(pred);
                }
                else {
                    this.baseLayers.remove(pred);
                }
            ...
            LayerManager.prototype.findLayer = function (name) {
                ...
                if (!layer) {
                    layer = LayerManagerHelper.findLayerViewModel(name, this.polygonLayers);
                }
                if (!layer) {
                    layer = LayerManagerHelper.findLayerViewModel(name, this.imageLayers);
                }
                if (!layer) {
            ...
            LayerManager.prototype.setWmsLayersFromUrl = function () {
                 ...
                 */
                // var ser = "http://app01.saeon.ac.za:8084/geoserver/DEA_Carbon/wms?service=WMS&version=1.1.0&request=GetMap&layers=DEA_Carbon:NetPrimaryProduction_gC_sq.m_yr_DEA_CSIR_1.1.9-2015-10-06&styles=&bbox=15.637660927790398,-35.02706985258885,33.65555257021669,-21.91846313931737&width=512&height=372&srs=EPSG:4326"
            ...
            LayerManager.prototype.populateWmsLayerFromUrl = function () {
                ...
                var self = this;
                var address = "http://app01.saeon.ac.za:8084/geoserver/DEA_Carbon/wms?service=WMS&version=1.1.0&request=GetMap&layers=DEA_Carbon:NetPrimaryProduction_gC_sq.m_yr_DEA_CSIR_1.1.9-2015-10-06&styles=&bbox=15.637660927790398,-35.02706985258885,33.65555257021669,-21.91846313931737&width=512&height=372&srs=EPSG:4326&format=image/geotiff"
                // The '.slice(1)' operation removes the question mark separator.
                // var urlParameters = new URLSearchParams(window.location.search.slice(1));
                var urlParameters = new URLSearchParams(address);
                        ...
                        addLayers: function (data, status, headers) {
                            if (status === "success") {
                                ...
                                for (i = 0; i < len; i++) {
                                    ...
                                    //     detailControl: config.imagerydetailControl
                                    // });
                                    self.addOverlayLayer(new EnhancedWmsLayer(wmsLayerConfig, null), {
                                        enabled: false,
                                        detailControl: config.imagerydetailControl
                                    });
                                    self.addWmsServer(wmsLayerConfig);
             ...
             * Add WMS layers from an associated WorldWind Server Kit (WWSK) server.
             */
            LayerManager.prototype.addAvailableWmsLayers2 = function (serveraddress,wmslayername,layername,enabled,opacity_val,category) {
                var requestUrl = serveraddress + "?SERVICE=WMS&VERSION=" + this.localWmsVersion + "&REQUEST=GetCapabilities";
                var self = this;
                var layerGenerator = function (myGlobe) {
                    var globe = myGlobe;
                    return {
                        addLayers: function (data, status, headers) {
                            if (status === "success") {
                                ...
                                for (i = 0; i < len; i++) {
                                    ...
                                    if(namedLayers[i].name==wmslayername) {
                                        // console.log(namedLayers[i].styles[0].legendUrls[0])
                                        wmsLayerConfig = WorldWind.WmsLayer.formLayerConfiguration(namedLayers[i]);

                                        var wwLayer = new EnhancedWmsLayer(wmsLayerConfig, null);
                                        // wwLayer.enabled = false;
                                        // if(enabled)
                                        // {
                                        //     wwLayer.enabled = true;
                                        // }else {
                                        ...
                                        if (namedLayers[i].styles && namedLayers[i].styles.length > 0
                                        ...
                                        if(layername==null)
                                        {
                                            ...
                                            wwLayer.displayName = layername; //Or, whatever you want it called.
                                        }
                                        if (category === constants.LAYER_CATEGORY_BASE) {
                                            self.addBaseLayer(wwLayer,{enabled: enabled, opacity: opacity_val});
                                        } else if (category === constants.LAYER_CATEGORY_OVERLAY) {
                                            self.addOverlayLayer(wwLayer,{enabled: enabled, opacity: opacity_val});
                                        } else if (category === constants.LAYER_CATEGORY_DATA) {
                                            self.addDataLayer(wwLayer,{enabled: enabled, opacity: opacity_val});
                                        } else if (category === constants.LAYER_CATEGORY_POLYGONS) {
                                            //still TODO
                                            //this.addPoLayer(wwLayer,{enabled: enabled, opacity: opacity_val});
                                        }else if (category === constants.LAYER_CATEGORY_IMAGE) {
                                            //still TODO
                                           // this.addDataLayer(wwLayer,{enabled: enabled, opacity: opacity_val});
                                        }
