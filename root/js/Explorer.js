...
define([
    ...
    function (
        ...
        var Explorer = function (wwd) {
            ...
            var clickRecognizer = new WorldWind.ClickRecognizer(this.wwd, function (event) {
                self.handleMouseclickEvent(event);
            });
            //double click
            clickRecognizer.numberOfClicks=2;
            window.addEventListener("click",clickRecognizer);
            ...
            var peaks = [
                {'name': "sfsdfdfsfsdf", 'elevation': 3159, 'latitude': -27, 'longitude': 27}
            ];
            var peak = peaks[0]
            var peakPosition = new WorldWind.Position(peak.latitude, peak.longitude, peak.elevation);
            var text = new WorldWind.GeographicText(peakPosition, peak.name + "\n" + peak.elevation + " m");
            ...
            textAttributes.offset = screenOffset;
            textLayer.addRenderable(text);
            textLayer.alwaysOnTop;
            textLayer.name='marc';
            // var ex = globe.layerManager.addBackgroundLayer()
            // Add the text layer to the WorldWindow's layer list.
            window.layermanager.addBackgroundLayer(textLayer, {
                enabled: true,
                hideInMenu: true,
                detailControl: config.imagerydetailControl
            });
            textLayer.renderables[0].text=""
         ...
         */
        Explorer.prototype.getglobelayermanager = function () {
            return this.globe.layerManager;
        };
...
//                            this.globe.updateEyePosition(); // update time widget
            ...
            try {
                ...
                if (pickList.objects.length > 0) {
                    ...
                    for (var p = 0; p < pickList.objects.length; p++) {
                        if (pickList.objects[p].userObject.layer.displayName == "NCCRD") {
                            textLayer.renderables[0].text=pickList.objects[p].userObject.displayName;
                            var position = pickList.objects[0].position
                            textLayer.renderables[0].position=position
                            pickList.objects[p].userObject.highlighted = true;
                            // console.log(pickList.objects[p].userObject.displayName)
                            // Explorer.textLayer.renderables[0].text="Some or other National project \n start date:"
                        }


                        // textLayer.renderables[0].text="Some or other National project \n start date:"
                        // var position = pickList.objects[0].position
                        // textLayer.renderables[0].position=position
                ...
                // if (pickList.objects.length === 1 && pickList.objects[0].isTerrain) {
                var position = pickList.objects[0].position;
                //     this.wwd.goTo(new WorldWind.Location(position.latitude, position.longitude));
                // }
        ...
        Explorer.prototype.handleMouseclickEvent = function (event) {
            var x = event.clientX,
                y = event.clientY;
            // var but = event.button.
            var pickList = this.wwd.pick(this.wwd.canvasCoordinates(x, y));
            if (this.isTouchDevice) {
                return; // ignore simulated mouse events in mobile browsers
            }
        ...
        // Explorer.prototype.wwd.deepPicking = true;
        ...
        Explorer.prototype.handleMouseclickEvent2 = function (event) {
            try {
            var x = event.clientX,
                y = event.clientY;
            // var but = event.button.
...
// De-highlight any previously highlighted placemarks.
            for (var h = 0; h < highlightedItems.length; h++) {
                highlightedItems[h].highlighted = false;
                // Explorer.textLayer.renderables[0].text=""
                redrawRequired = true;
            }
            highlightedItems = [];
